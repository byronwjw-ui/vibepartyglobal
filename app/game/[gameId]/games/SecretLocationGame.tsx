...
export default function SecretLocationGame() {
  ...
  return (
    <GameLayout title="间谍地点 🗺️" rules="1 名间谍不知道地点；其他人通过问答找出间谍，间谍要猜地点。">
      <AnimatePresence mode="wait">
        {phase === 'setup' && (
          <motion.div key="s" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="py-6 space-y-4 text-center">
            <div className="text-6xl">🗺️</div>
            <div className="font-bold text-paper-900/75">将随机选定一个地点，其中 1 人为间谍。</div>
        ...
        {phase === 'view' && (
          <motion.div key={'v' + viewIdx} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="py-6 space-y-4 text-center">
            ...
            <div className="sticker p-8 min-h-[180px] grid place-items-center bg-paper-50">
              {!shown ? (
                <div><div className="font-bold text-paper-900/70 mb-3">手机传给 {players[viewIdx]?.name}</div><NeonButton onClick={() => setShown(true)}>查看我的身份</NeonButton></div>
              ) : viewIdx === spyIdx ? (
                <div>
                  <div className="text-6xl mb-2">🕶️</div>
                  <div className="text-2xl font-black doodle-title">你是间谍</div>
                  <div className="text-xs font-bold text-paper-900/70 mt-1">请伪装并猜出地点</div>
                </div>
              ) : (
                <div>
                  <div className="text-xs font-bold text-paper-900/70">今晚的地点</div>
                  <div className="text-3xl font-black doodle-title mt-2">{location}</div>
                </div>
              )}
            </div>
        ...
        {phase === 'ask' && (
          <motion.div key="a" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="py-4 space-y-4">
            <div className="sticker p-5 text-center bg-sticker-yellow">
              <div className="font-bold text-paper-900/85 text-sm">玩家轮流互相提问，不能直接问“你在哪”。</div>
              <div className="text-xs font-bold text-paper-900/60 mt-1">建议每轮 60 秒，问完进入投票。</div>
            </div>
            <div className="grid place-items-center"><CountdownTimer seconds={60} running /></div>
        ...
        {phase === 'vote' && (
          <motion.div key="vo" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="py-4 space-y-4">
            <div className="text-sm font-bold text-paper-900/70">选出你们怀疑的间谍</div>
        ...
        {phase === 'reveal' && (
          <motion.div key="r" initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="py-6 space-y-4 text-center">
            <div className="text-6xl">🕶️</div>
            <div className="font-bold text-paper-900/70">真正的间谍是</div>
            <div className="text-3xl font-black doodle-title">{spyPlayer?.name}</div>
            <div className="sticker p-4 text-sm font-semibold bg-paper-50">
              ...
              <div className="mt-2 font-bold text-paper-900/75">{suspect?.id === spyPlayer?.id ? '✅ 平民阵营猜中' : '❌ 间谍逃过一劫'}</div>
            </div>
